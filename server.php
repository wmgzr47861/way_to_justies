...
session_start();
	...
	{
		$username=$_POST['name'];    // get the values from php file //
		$email=$_POST['email'];
		$mob=$_POST['mobno'];
		$gen=$_POST['gender'];
		$pwd1=$_POST['pw1'];
		...
		{
			$password=md5($pwd1);
			$sql="INSERT INTO users (username,email,mob,gen,password) VALUES ('$username','$email','$mob','$gen','$password')";  //insert the values into databse//
			mysqli_query($db,$sql);
	...
	/*if(isset($_POST['login']))
	{
		...
		$sql= "SELECT username FROM users WHERE username='$username'";    // check the email id already exist or not //
		$check=mysqli_query($db,$sql);
